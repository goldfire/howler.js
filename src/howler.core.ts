...
import { globalPluginManager, type HowlerPlugin } from "./plugins";
import {
		...
		if (parent._webAudio && Howler.ctx) {
			const gainNode =
				typeof Howler.ctx.createGain === "undefined"
					? (
							Howler.ctx as { createGainNode?: () => GainNode }
						).createGainNode?.()
	...
	play(sprite?: string | number, internal?: boolean): number | null {
		...
		if (this._webAudio && node && isGainNode(node)) {
			...
			const loadedNoReadyState =
				typeof (window as WindowWithAudio).ejecta !== "undefined" ||
				(!node.readyState && Howler._navigator && Howler._navigator.isCocoonJS);

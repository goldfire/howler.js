...
export class PluginManager {
	...
	register(plugin: HowlerPlugin): void {
		...
		if (this.howlerInstance && hooks.onHowlerInit) {
			try {
				...
				console.error(
					`Error during onHowlerInit for plugin "${plugin.name}":`,
					error,
				);
			}
		}
	...
	unregister(pluginName: string): void {
		...
		if (registered.plugin.onUnregister) {
			try {
				...
				console.error(
					`Error during onUnregister for plugin "${pluginName}":`,
					error,
				);
			}
		}
	...
	private _executeHooks(
		hookName: string,
		callback: (hooks: PluginHooks) => void,
	): void {
		for (const [pluginName, registered] of this.plugins) {
			try {
				...
				console.error(
					`Error in hook "${hookName}" for plugin "${pluginName}":`,
					error,
				);
			}
		}
